...
func (s *Service) Checkpoint(ctx context.Context, principal auth.Principal, jobID, token, checkpoint string, version int64) (domain.TrainingJob, error) {
	...
	err := s.db.Write(operationCtx, func(tx *sql.Tx) error {
		job, err := s.repo.FindJob(operationCtx, tx, principal.TenantID, jobID)
		if err != nil {
			return err
		}
		if job.Owner != principal.UserID || job.LeaseToken != token || job.Version != version {
		...
		if err := s.repo.SaveCheckpoint(operationCtx, tx, principal.TenantID, jobID, principal.UserID, token, checkpoint, version, now); err != nil {
			return err
		}
