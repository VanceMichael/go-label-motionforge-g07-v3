...
func (s *Service) Complete(ctx context.Context, principal auth.Principal, jobID, token, outputURI, requestID string, version int64) (domain.TrainingJob, error) {
	...
	now := s.clock.Now()
	if _, err := s.repo.FindJob(ctx, s.db.SQL(), principal.TenantID, jobID); err != nil {
		return domain.TrainingJob{}, err
	}
	if err := s.repo.Complete(ctx, s.db.SQL(), principal.TenantID, jobID, principal.UserID, token, outputURI, version, now); err != nil {
		return domain.TrainingJob{}, err
	}
