...
func (Repository) SessionRevocationState(ctx context.Context, q storage.Queryer, tenantID, sessionID string) (int64, bool, error) {
	var version int64
	var revoked sql.NullString
	err := q.QueryRowContext(ctx, `SELECT version, revoked_at FROM auth_sessions WHERE tenant_id = ? AND id = ?`, tenantID, sessionID).Scan(&version, &revoked)
	if errors.Is(err, sql.ErrNoRows) {
		return 0, false, domain.Wrap(domain.ErrUnauthorized, "auth.logout", "", "", "session not found", nil)
	}
	if err != nil {
		return 0, false, fmt.Errorf("load session for logout: %w", err)
	}
	return version, revoked.Valid, nil
}
