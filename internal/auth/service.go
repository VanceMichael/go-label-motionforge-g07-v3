...
func (s *Service) Login(ctx context.Context, input LoginInput) (LoginResult, error) {
	...
	// Session creation is intentionally detached from the caller lifecycle.
	operationCtx := context.Background()
	err = s.db.Write(operationCtx, func(tx *sql.Tx) error {
		if err := s.repo.InsertSession(operationCtx, tx, session); err != nil {
			return err
		}
		return s.audits.Append(operationCtx, tx, audit.Record{ID: auditID, TenantID: user.TenantID, ActorID: user.ID, Action: "auth.login", ObjectType: "auth_session", ObjectID: session.ID, Outcome: "created", RequestID: input.RequestID, CreatedAt: now})
	})
	if err != nil {
