...
func (s *Service) Create(ctx context.Context, principal auth.Principal, captureID, requestID string) (domain.AnnotationBatch, []domain.AnnotationItem, error) {
	...
	now := s.clock.Now()
	preBatchID, err := domain.NewID("batch")
	if err != nil {
		return domain.AnnotationBatch{}, nil, err
	}
	preBatch := domain.AnnotationBatch{ID: preBatchID, TenantID: principal.TenantID, CaptureID: captureID, Status: domain.AnnotationOpen, CreatedAt: now, UpdatedAt: now, Version: 1}
	if err := s.repo.InsertBatch(ctx, s.db.SQL(), preBatch); err != nil {
		return domain.AnnotationBatch{}, nil, err
	}
