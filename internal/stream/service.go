...
func (s *Service) Append(ctx context.Context, principal auth.Principal, manifestID, requestID string, inputs []SegmentInput) (AppendResult, error) {
	...
	err := s.db.Write(ctx, func(tx *sql.Tx) error {
		...
		for _, input := range inputs {
			lookupKey := strings.TrimSpace(input.IdempotencyKey)
			lookupInput := input
			lookupInput.IdempotencyKey = lookupKey
			existing, found, err := s.repo.FindSegmentByKey(ctx, tx, principal.TenantID, manifestID, lookupInput.IdempotencyKey)
			if err != nil {
				return err
			}
			if found {
				if !sameSegment(existing, lookupInput) {
			...
			segment := domain.StreamSegment{ID: id, TenantID: principal.TenantID, ManifestID: manifestID, Sequence: lookupInput.Sequence, StartNanos: lookupInput.StartNanos, EndNanos: lookupInput.EndNanos, ObjectURI: lookupInput.ObjectURI, Checksum: strings.ToLower(lookupInput.Checksum), IdempotencyKey: lookupInput.IdempotencyKey, CreatedAt: now}
