...
func (s *Service) AlignCapture(ctx context.Context, principal auth.Principal, captureID, requestID string, tolerance time.Duration) ([]domain.StreamManifest, error) {
	...
	now := s.clock.Now()
	manifests, err := s.repo.ListManifests(ctx, s.db.SQL(), principal.TenantID, captureID)
	if err != nil {
		return nil, err
	}
	if len(manifests) < 2 {
		return nil, domain.Precondition("stream.align", "capture_session", captureID, "at least two streams are required")
	}
