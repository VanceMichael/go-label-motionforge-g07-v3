...
func (s *Service) Submit(ctx context.Context, principal auth.Principal, captureID, requestID string) (domain.CaptureSession, error) {
	...
	now := s.clock.Now()
	current, err := s.repo.Find(ctx, s.db.SQL(), principal.TenantID, captureID)
	if err != nil {
		return domain.CaptureSession{}, err
	}
	...
	if principal.Role == domain.RoleOperator && current.OperatorID != principal.UserID {
		return domain.CaptureSession{}, domain.Wrap(domain.ErrForbidden, "capture.cancel", "capture_session", captureID, "operator does not own capture", nil)
	}
	lease, leaseErr := s.facilities.FindActiveLease(ctx, s.db.SQL(), principal.TenantID, current.RigID)
	if leaseErr == nil {
		if lease.CaptureID != captureID {
			return domain.CaptureSession{}, domain.Conflict("capture.cancel", "capture_session", captureID, "rig lease belongs to another capture")
		}
		if err := s.facilities.ReleaseLease(ctx, s.db.SQL(), principal.TenantID, lease.ID, lease.Owner, lease.Token, lease.Version, now); err != nil {
			return domain.CaptureSession{}, err
		}
	} else if !errors.Is(leaseErr, domain.ErrNotFound) {
		return domain.CaptureSession{}, leaseErr
	}
