...
func (s *Service) transition(ctx context.Context, principal auth.Principal, captureID, requestID string, from, to domain.CaptureStatus, times TransitionTimes, action string, roles ...domain.Role) (domain.CaptureSession, error) {
	...
	var updated domain.CaptureSession
	operationCtx := ctx
	if to == domain.CaptureReady {
		operationCtx = context.Background()
	}
	err := s.db.Write(operationCtx, func(tx *sql.Tx) error {
		current, err := s.repo.Find(operationCtx, tx, principal.TenantID, captureID)
		if err != nil {
			return err
		}
		...
		if err := s.repo.Transition(operationCtx, tx, principal.TenantID, captureID, from, to, current.Version, now, times); err != nil {
			return err
		}
		if err := s.appendEffects(operationCtx, tx, principal, requestID, action, captureID, string(to), now); err != nil {
			return err
		}
