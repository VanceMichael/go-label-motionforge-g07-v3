...
./build_benzhi_docker.sh benzhi-task-294-amd64 linux/amd64
./build_benzhi_docker.sh benzhi-task-294-arm64 linux/arm64
docker run -it benzhi-task-294-amd64:latest
docker run -it --platform linux/arm64 benzhi-task-294-arm64:latest
```

## 题目验证命令

1. 预期退出码 1：`go test ./integration -run '^TestTask0002CanceledLoginLeavesNoSession$' -count=1`
